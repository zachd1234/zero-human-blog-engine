...
public class WordPressBlockClient extends BaseWordPressClient {
    ...
    public void updateHeadingAndSubheading(String heading, String subheading) throws IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(getRequest)) {
            ...
            // Replace blocks one at a time
            Pattern headingRegex = Pattern.compile(headingPattern);
            Pattern subheadingRegex = Pattern.compile(subheadingPattern);
            
            java.util.regex.Matcher headingMatcher = headingRegex.matcher(currentContent);
            String tempContent = headingMatcher.replaceFirst(newHeading);
            
            // Debug subheading replacement
            java.util.regex.Matcher debugMatcher = subheadingRegex.matcher(tempContent);
            if (debugMatcher.find()) {
                System.out.println("Found subheading block to replace: " + debugMatcher.group(0));
            } else {
                System.out.println("WARNING: Subheading block not found!");
                System.out.println("Pattern: " + subheadingPattern);
                System.out.println("Content: " + tempContent);
            }
            
            java.util.regex.Matcher subheadingMatcher = subheadingRegex.matcher(tempContent);
            String updatedContent = subheadingMatcher.replaceFirst(newSubheading);
