...
public class WordPressBlockClient extends BaseWordPressClient {
    ...
    public void updatePageSection(int pageId, String sectionId, String newContent) throws IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(getRequest)) {
            ...
            // Find and replace the entire Features section including its content
            String targetBlockPattern = 
                "<!-- wp:group \\{\"metadata\":\\{\"name\":\"Features\"\\}[\\s\\S]*?<!-- /wp:group -->\\s*<!-- /wp:group -->";
            
            // Wrap new content in the proper group container
            String wrappedNewContent = String.format(
            ...
            System.out.println("1. Found matching section: " + matcher.find());
            
            String updatedContent = currentContent.replaceFirst(targetBlockPattern, wrappedNewContent);
            System.out.println("2. Content changed: " + !updatedContent.equals(currentContent));
            ...
            try (CloseableHttpResponse updateResponse = httpClient.execute(updateRequest)) {
                int statusCode = updateResponse.getCode();
                System.out.println("3. Update status code: " + statusCode);
            }
