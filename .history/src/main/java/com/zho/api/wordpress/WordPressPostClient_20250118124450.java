...
public class WordPressPostClient extends BaseWordPressClient {
    ...
    public PostResponse publishPost(BlogPost blogPost) throws IOException {
        ...
        public List<Integer> getAllPostIds() throws IOException {
        Request request = new Request.Builder()
            .url(apiUrl + "/posts?per_page=100&fields=id")  // Only get IDs, 100 at a time
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                throw new IOException("Failed to get posts: " + response);
            }
    ...
    public void deletePost(int postId) throws IOException {
        Request request = new Request.Builder()
            .url(apiUrl + "/posts/" + postId + "?force=true")  // force=true permanently deletes
            .addHeader("Authorization", "Bearer " + apiToken)
            .delete()
            .build();
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                throw new IOException("Failed to delete post " + postId + ": " + response);
            }
        }
    }
