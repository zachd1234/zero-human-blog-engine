...
public class WordPressBlockClient extends BaseWordPressClient {
    ...
    public void updatePageSection(int pageId, String sectionId, String newContent) throws IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(getRequest)) {
            ...
            // Debug prints
            System.out.println("\n=== DEBUG INFO ===");
            System.out.println("1. Current content length: " + currentContent.length());
            System.out.println("2. New content to insert length: " + newContent.length());
            
            // Find our target block and replace just that one
            String targetBlockPattern = "<!-- wp:group \\{\"metadata\":\\{\"name\":\"Features\"\\}[^}]*\\}[\\s\\S]*?<!-- /wp:group -->";
            
            // Debug regex match
            java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(targetBlockPattern);
            java.util.regex.Matcher matcher = pattern.matcher(currentContent);
            System.out.println("3. Found matching section: " + matcher.find());
            if (matcher.find()) {
                System.out.println("4. Matched content: " + matcher.group());
            }
            
            String updatedContent = currentContent.replaceFirst(targetBlockPattern, newContent);
            
            // Debug content change
            System.out.println("5. Content changed: " + !updatedContent.equals(currentContent));
            System.out.println("6. Updated content length: " + updatedContent.length());
            ...
            try (CloseableHttpResponse updateResponse = httpClient.execute(updateRequest)) {
                int statusCode = updateResponse.getCode();
                String updateResponseBody = EntityUtils.toString(updateResponse.getEntity());
                System.out.println("7. Update status code: " + statusCode);
                System.out.println("8. Response body: " + updateResponseBody);
            }
