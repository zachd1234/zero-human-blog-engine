...
public class WordPressBlockClient extends BaseWordPressClient {
    ...
    public void updatePageSection(int pageId, String sectionId, String newContent) throws IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(getRequest)) {
            ...
            // Look for the Features section between Blog and Mission sections
            String targetPattern = "<!-- wp:group \\{\"metadata\":\\{\"name\":\"Blog\"\\}[\\s\\S]*?<!-- wp:group \\{\"metadata\":\\{\"name\":\"Features\"\\}[\\s\\S]*?<!-- wp:group \\{\"metadata\":\\{\"name\":\"Blog\"\\}";
            ...
            if (matcher.find()) {
                ...
                // Extract the first Blog section and the Features section
                String firstPart = currentContent.substring(0, start);
                String lastPart = currentContent.substring(end - "<!-- wp:group {\"metadata\":{\"name\":\"Blog\"}".length());
                
                // Combine with new content
                String updatedContent = firstPart + newContent + lastPart;
                ...
                System.out.println("Could not find Features section between Blog sections");
            }
        }
    }
