...
public class WordPressPostClient extends BaseWordPressClient {
    ...
    public String publishPost(BlogPost blogPost) throws IOException {
        ...
        if (blogPost.getCoverImage() != null) {
            // First upload the image and get media ID
            int mediaId = uploadFeaturedImage(blogPost.getCoverImage().getUrl());
            if (mediaId > 0) {
                postData.put("featured_media", mediaId);
            }
        }
    ...
    private int uploadFeaturedImage(String imageUrl) throws IOException {
        Map<String, Object> mediaData = new HashMap<>();
        mediaData.put("url", imageUrl);
        
        String response = makeRequest("POST", "media", mediaData);
        JSONObject jsonResponse = new JSONObject(response);
