...
public class WordPressBlockClient extends BaseWordPressClient {
    ...
    public void updateTopicsSection(List<Topic> topics) throws IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(getRequest)) {
            ...
            // Create the new topics section HTML
            StringBuilder newTopicsSection = new StringBuilder();
            // Start main container
            newTopicsSection.append("<!-- wp:group {\"metadata\":{\"name\":\"Features\"},\"className\":\"nfd-container nfd-p-lg nfd-theme-darker nfd-bg-surface nfd-wb-features__features-4 is-style-nfd-theme-darker\",\"layout\":{\"type\":\"constrained\"}} -->\n");
            newTopicsSection.append("<div class=\"nfd-container nfd-p-lg nfd-theme-darker nfd-bg-surface nfd-wb-features__features-4 is-style-nfd-theme-darker wp-block-group nfd-bg-effect-dots\">\n");
            
            // Add heading
            newTopicsSection.append("<!-- wp:heading {\"textColor\":\"theme-palette9\"} -->\n");
            newTopicsSection.append("<h2 class=\"wp-block-heading has-theme-palette-9-color has-text-color\">Explore Topics</h2>\n");
            newTopicsSection.append("<!-- /wp:heading -->\n");
            
            // Start topics container
            newTopicsSection.append("<!-- wp:group {\"layout\":{\"type\":\"flex\",\"orientation\":\"vertical\"}} -->\n");
            newTopicsSection.append("<div class=\"wp-block-group\">\n");
            
            // Create rows of 3 topics each
            for (int i = 0; i < topics.size(); i += 3) {
                newTopicsSection.append("<!-- wp:columns {\"className\":\"nfd-text-base nfd-gap-md nfd-gap-y-xl\"} -->\n");
                newTopicsSection.append("<div class=\"nfd-text-base nfd-gap-md nfd-gap-y-xl wp-block-columns\">\n");
                
                // Add up to 3 topics in this row
                for (int j = i; j < Math.min(i + 3, topics.size()); j++) {
                    Topic topic = topics.get(j);
                    newTopicsSection.append(createTopicCard(topic, j));
                }
                
                newTopicsSection.append("</div>\n<!-- /wp:columns -->\n");
            }
            ...
            String topicsSectionPattern = "<!-- wp:group \\{[^}]*\"name\":\"Features\"[^}]*\\}[\\s\\S]*?<!-- /wp:group -->";
            String updatedContent = currentContent.replaceAll(topicsSectionPattern, newTopicsSection.toString());
            
            // Update only if we found and replaced the section
            if (!updatedContent.equals(currentContent)) {
                ...
                try (CloseableHttpResponse updateResponse = httpClient.execute(updateRequest)) {
                    int statusCode = updateResponse.getCode();
                    System.out.println("Topics section update status: " + statusCode);
                }
            } else {
                System.out.println("Warning: Topics section not found in the page content");
            }
        }
    }
