...
public class KeywordResearchService {
    ...
    private List<String> generateSeedKeywords(BlogRequest blogRequest) {
        ...
        String prompt = String.format(
            "Convert this topic '%s' into a comprehensive, natural-sounding search phrase that someone might " +
            "use when looking for in-depth information. Make it specific but not too long. " +
            "Don't use special characters or quotes. Just return the phrase itself.",
            blogRequest.getTopic()
        );
        
        String enhancedSeedKeyword = chatGPTService.generateContent(prompt);
