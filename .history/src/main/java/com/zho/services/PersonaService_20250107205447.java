...
public class PersonaService {
    ...
    private String generateStereotypicalDescription(String topic) throws IOException {
        String stereotypePrompt = String.format(
            "For a blog about %s, describe the most stereotypical person who would be an expert in this field.\n\n" +
            "Format your response as a detailed image prompt following this template:\n" +
            "\"Create a hyper-realistic image of a [specific age group, gender, and appearance fitting the blog's typical creator]. " +
            "They are [describe grooming/style typical for the role] and dressed in [attire appropriate for the blog's subject matter], " +
            "exuding [key traits, e.g., professionalism, enthusiasm, creativity]. " +
            "The setting reflects their area of expertise, featuring [relevant props, tools, or background elements]. " +
            "Their expression is [describe demeanor, e.g., confident, approachable, focused], emphasizing their " +
            "[knowledge, passion, or personality traits relevant to the blog topic].\"\n\n" +
            "Return only the completed image prompt, filling in all the bracketed sections appropriately for %s expertise.",
            topic,
            topic
        );
    ...
    public static void main(String[] args) {
        try {
            ...
            personaService.generateAndSetupPersona("tennis blog");
