...
public class KeywordResearchService {
    ...
    private double parseAIResponse(String analysis) {
        try {
            ...
            // Look for lines containing numbers (1-10)
            for (String line : lines) {
                if (line.matches(".*\\d+/10.*")) {  // Look for format "X/10"
                    // Extract the number before "/10"
                    String number = line.replaceAll(".*?(\\d+)/10.*", "$1");
            ...
            System.err.println("Raw response: " + analysis);
            return 0.0;
        }
    }
