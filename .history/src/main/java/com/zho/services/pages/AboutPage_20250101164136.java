...
public class AboutPage implements StaticPage {
    ...
    private void updateStoryAndMission(String story, String mission) throws IOException, ParseException {
        // Update story using simple paragraph
        blockClient.updateBlock(
            getPageId(),
            WordPressBlockClient.BlockType.SIMPLE_PARAGRAPH,
            null,
            story,
            new JSONObject().put("className", "story")
        );

        // Update mission using simple paragraph
        blockClient.updateBlock(
            getPageId(),
            WordPressBlockClient.BlockType.SIMPLE_PARAGRAPH,
            null,
            mission,
            new JSONObject().put("className", "mission")
        );
    }
