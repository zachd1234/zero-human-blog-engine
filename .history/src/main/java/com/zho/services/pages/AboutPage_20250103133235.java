...
public class AboutPage implements StaticPage {
    ...
    public AboutPage(WordPressBlockClient blockClient, OpenAIClient openAIClient, WordPressMediaClient mediaClient) {
    ...
    private void updateAboutImage(UnsplashImage image) throws IOException, ParseException {
        mediaClient.updateInfoboxImage(pageId, "318_1fa743-7a", image);
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            AboutPage aboutPage = new AboutPage(blockClient, openAIClient, mediaClient);
            ...
            System.out.println("Fetching test image from Unsplash...");
            List<UnsplashImage> testImages = unsplashClient.searchImages("tennis", 1);
            
            if (testImages.isEmpty()) {
                throw new RuntimeException("Failed to fetch test image from Unsplash");
            }
            
            // Test the page update
            System.out.println("Testing About page update...");
            System.out.println("Using image: " + testImages.get(0).getUrl());
            aboutPage.updateStaticContent(testRequest, testImages);
