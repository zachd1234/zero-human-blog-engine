...
public class KeywordResearchService {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("\nFormat: Keyword | Monthly Searches | Competition");
            System.out.println("------------------------------------------------------------");
            
            keywords.stream()
                .limit(20) // Show first 20 keywords
                .forEach(result -> {
                    System.out.printf("%-30s | %14d | %11s%n",
                        result.getText(),
                    ...
                    if (keywords.indexOf(result) == 0) {
                        System.out.println("\nAvailable methods on result:");
                        Arrays.stream(result.getClass().getMethods())
                            .forEach(method -> System.out.println("- " + method.getName()));
                        
                        System.out.println("\nAvailable methods on KeywordIdeaMetrics:");
                        Arrays.stream(result.getKeywordIdeaMetrics().getClass().getMethods())
                            .forEach(method -> System.out.println("- " + method.getName()));
                    }
                });
