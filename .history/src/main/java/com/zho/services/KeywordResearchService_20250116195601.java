...
public class KeywordResearchService {
    ...
    public List<KeywordAnalysis> getLongTailKeywords(BlogRequest blogRequest, int keywordCount) {
        try {
            ...
            // Filter keywords with low competition
            for (GenerateKeywordIdeaResult keyword : allKeywords) {
                if (keyword.getKeywordIdeaMetrics().getCompetitionLevel().toString().equals("LOW") || 
                    keyword.getKeywordIdeaMetrics().getCompetitionLevel().toString().isEmpty()) {
