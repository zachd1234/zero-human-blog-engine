...
public class BlogPostService {
    ...
    private List<BlogPost> generateBlogPosts(BlogRequest request, int postCount, int[] postIds) throws IOException, ParseException {
        ...
        for (int i = 0; i < postCount; i++) {
            ...
            Image image = unsplashClient.searchImages(searchTerm).get(0);
    ...
    private void updateWordPressPosts(List<BlogPost> posts) throws IOException, ParseException {
        for (BlogPost post : posts) {
            ...
            int categoryId = wpClient.categories().getCategoryIdByName(post.getCategory());
            wpClient.posts().updatePostCategory(post.getId(), categoryId);
            wpClient.media().updatePostCoverImage(post.getId(), post.getCoverImage());
        }
    }
