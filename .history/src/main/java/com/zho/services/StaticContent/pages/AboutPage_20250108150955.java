...
public class AboutPage implements StaticPage {
    ...
    private void updateMission(String intro, String mission) throws IOException, ParseException {
        ...
        // Update mission heading and content
        String missionPattern = "<!-- wp:heading \\{\"textAlign\":\"center\",\"uniqueID\":\"318_mission\"\\} -->[\\s\\S]*?<!-- /wp:paragraph -->";
        String newMission = "<!-- wp:heading {\"textAlign\":\"center\",\"uniqueID\":\"318_mission\"} -->\n" +
                           "<h2 class=\"wp-block-heading has-text-align-center\">Our Mission</h2>\n" +
                           "<!-- /wp:heading -->\n\n" +
                           "<!-- wp:paragraph -->\n<p class=\"\">" + mission + "</p>\n<!-- /wp:paragraph -->";
        
        String updatedContent = currentContent.replaceFirst(missionPattern, newMission);
        blockClient.updatePageContent(getPageId(), updatedContent);
    }
